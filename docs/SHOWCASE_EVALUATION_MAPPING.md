...
| 3 | **Security & compliance** | `docs/audit/THREAT_MODEL.md`, `docs/audit/PENTEST_SCOPE.md`, `docs/runbooks/dppa-breach-notification.md`, `docs/CUSTODY.md` (3-of-5 multi-sig), `backend/app/auth/jwt_auth.py` (PyJWT — ecdsa CVE eliminated), `backend/app/crypto.py` (AES-GCM for NIN), `CHANGELOG.md` (CVE timeline), `docs/adr/0002-zero-trust-posture.md` | CycloneDX SBOMs committed in `evidence/sbom/`; pre-engagement pentest SoW (`docs/audit/PENTEST_SCOPE.md`); 72-h DPPA §19 runbook with PDPO + SMS templates; Caddy CSP/STS verified; `Settings.assert_prod_safety()` refuses dev defaults; Foundry `test_DirectAnchorBypassFails` proves no single key can anchor | **5/5** |
...
| 5 | **Usability & accessibility** | `docs/DESIGN_SYSTEM.md`, `docs/IMPACT_EVIDENCE.md`, `frontend/e2e/accessibility.spec.ts` (formal axe-core assertion), `frontend/src/app/(public)/verify/page.tsx`, `backend/app/routers/ussd.py`, `docs/USSD_DEPLOYMENT.md` | `evidence/lighthouse/20260525T143155Z/` baseline (committed perf/a11y/best-pr/SEO scores + SUMMARY.md); `bash scripts/lighthouse_ci.sh` re-runs against any deployed instance; `bun run e2e` in the `accessibility` CI job asserts 0 critical / 0 serious axe-core violations on six citizen-critical routes; USSD pathway verified on Africa's Talking simulator (`backend/tests/test_ussd.py`); WCAG 2.2 AA per page; offline PWA fallback | **5/5** |
...
- Pen-test scope: [`docs/audit/PENTEST_SCOPE.md`](./audit/PENTEST_SCOPE.md)
- DPPA breach runbook: [`docs/runbooks/dppa-breach-notification.md`](./runbooks/dppa-breach-notification.md)
- Accessibility spec: [`frontend/e2e/accessibility.spec.ts`](../frontend/e2e/accessibility.spec.ts)
...
- Changelog (CVE + audit-grade-change timeline): [`CHANGELOG.md`](../CHANGELOG.md)
